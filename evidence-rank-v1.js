...
(function(){
  ...
  let minSample=0,minWin=0,rankMode='quality';
  let latest=[];
  ...
  if(!panel)return;
  const table=panel.querySelector('.table-wrap');const tools=document.createElement('div');tools.className='evidence-rank-tools';tools.innerHTML=`<select id="evidenceRank"><option value="quality">Rank: Evidence quality</option><option value="win10">Rank: +10D success</option><option value="median5">Rank: +5D median</option><option value="sample">Rank: Sample size</option></select><select id="minEvidenceSample"><option value="0" selected>Any sample</option><option value="10">n ≥ 10</option><option value="30">n ≥ 30</option><option value="100">n ≥ 100</option><option value="500">n ≥ 500</option></select><select id="minEvidenceWin"><option value="0" selected>Any success rate</option><option value="50">Success ≥ 50%</option><option value="55">Success ≥ 55%</option><option value="60">Success ≥ 60%</option><option value="65">Success ≥ 65%</option></select><span class="evidence-rank-note" id="evidenceRankNote">Showing all scan matches; evidence filters are optional.</span>`;panel.insertBefore(tools,table);
  ...
  function apply(rows){const loaded=rows.some(r=>r?.evidence_detail);let out=[...rows];if(loaded){out=out.filter(r=>{const s=sample(r),w=success(r,2);return s>=minSample&&(minWin===0||w===null||w>=minWin)});out.sort((a,b)=>score(b)-score(a));const filtered=minSample>0||minWin>0;document.querySelector('#evidenceRankNote').textContent=filtered?`Evidence filters • ${out.length.toLocaleString()} of ${rows.length.toLocaleString()} matches shown`:`Showing all ${out.length.toLocaleString()} scan matches • ranked by evidence quality`; }return out}
